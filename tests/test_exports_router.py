...
def test_precomputed_csv_endpoints_missing(client, tmp_path, monkeypatch):
    """Point the router at an empty tables dir to test the 404 path
    regardless of what artefacts the developer happens to have on disk."""
    from src.api.routers import exports as exports_module

    monkeypatch.setattr(exports_module, "_tables_dir", lambda: tmp_path)
